...
4. Launch 7674_pgc_fit.py to train and copy the latest model from checkpoint
...
def run_text_to_binary_dataset(input_file):
    ...
    # The output path that 7674_pgc_fit.py expects
...
def run_pgc_jam(output_file, mode='jam', fit_args=None):
    """
    Launch 7674_pgc_fit.py to train on the dataset and copy the latest model.
    Includes all stopping criteria, including train_incorrect_stop.
    
    Args:
        output_file (str): Path to the output file
        mode (str): 'jam' or 'fit'
        fit_args (list): Extra CLI args for 7674_pgc_fit.py
    """
    print(f"Running 7674_pgc_fit.py with mode={mode}...")
    from datetime import datetime
    ...
    # Build base args
    base_args = [
        sys.executable, '7674_pgc_fit.py', 
...
def pack_file(input_file, output_file=None, encoding='base64', mode='jam', fit_args=None):
    ...
    2. Prepend 98 '@' characters to the encoded data
    3. Launch text_to_binary_dataset.py with the modified data as input
    4. Launch 7674_pgc_fit.py to train and copy the latest model
    ...
    Args:
        ...
        fit_args (list, optional): Extra CLI args for 7674_pgc_fit.py
    """
    ...
    try:
        ...
        # Step 4: Run 7674_pgc_fit.py and copy the latest model, passing any extra fit_args
        print("Step 4: Running 7674_pgc_fit.py and copying latest model...")
...
def main():
    ...
    parser.add_argument('--mode', default='jam', choices=['jam', 'fit'], help="Mode for 7674_pgc_fit.py: jam or fit (default: jam)")
